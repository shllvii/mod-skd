...
void print_skd(struct MSkdSolution *solution) {
...
void print_buff(struct BuffSolution *solution) {
  ...
  for (int i=0; i<solution->ND; i++) {
    LOG("\t(%d -> %d: %d)", solution->from[i], solution->to[i], solution->length[i]);
  }
  LOG("\n");
...
int solve_delta(struct CycleT x, struct CycleT y, int *delta, char *name) {
  ...
  try {
    ...
    for (int i=0; i< x.N; i++) {
      vars[i] = model.addVar(0, x.n[i]-1, 0, GRB_INTEGER, "k_x_"+std::to_string(i));
      obj += vars[i] * (-x.I[i]);
    }
    for (int i=0; i<y.N; i++) {
      vars[x.N+i] = model.addVar(0, y.n[i]-1, 0, GRB_INTEGER, "k_y_" + std::to_string(i));
...
int solve_starttime(struct SDC *sdc, struct MSkdSolution *solution) {
  print_sdc(sdc);
  int *dist = (int *)malloc(sdc->N * sizeof(int));
...
int solve_buff(struct CycleT x, struct CycleT y, int tx, int ty, int *len, char *name) {
  ...
  */
  ...
  try {
    ...
    LOG("\tadd variables:");
    for (int i=0; i< x.N; i++) {
      kx[i] = model.addVar(0, x.n[i]-1, 0, GRB_INTEGER, "k_x_"+std::to_string(i));
      obj += kx[i] * x.n[i];
    }
    for (int i=0; i<y.N; i++) {
      ky[i] = model.addVar(0, y.n[i]-1, 0,  GRB_INTEGER, "k_y_" + std::to_string(i));
      obj -= ky[i] * y.n[i];
    }
    ...
    if (status == GRB_OPTIMAL)
    {
      ...
      for (int i=0; i<x.N + y.N; i++) {
        LOG("\tvar %s = %f\n", vars[i].get(GRB_StringAttr_VarName).c_str(), vars[i].get(GRB_DoubleAttr_X));
      }
    } else
    if ((status != GRB_INF_OR_UNBD) && (status != GRB_INFEASIBLE))
    {
...
int mod_skd(struct ModSys *ms, struct MSkdSolution *solution) {
  ...
  for (int i = 0; i < ND; i++) {
    ...
    int status = solve_delta(ms->cycle[from], ms->cycle[to], delta + i, name);
  ...
  sdc.sink = T;
  for (int i = 0; i < ND; i++) {
    int from_m = ms->mp[ms->from[i]];
    int to_m = ms->mp[ms->to[i]];
    int found = -1;
    for (int j = 0; j < sdc.M; j++) {
    ...
    sdc.delta[found] = min(sdc.delta[found], delta[i]);
  }
  ...
  if (status) {
    fprintf(stderr, "ERRORs when solving SDC\n");
    return 1;
  }
...
int buff_opt(struct ModSys *ms, struct MSkdSolution *skd, struct BuffSolution *solution) {
  ...
  for (int i=0; i<ms->ND; i++) {
    ...
    solution->to[i] = to;
    int status = solve_buff(ms->cycle[from], ms->cycle[to], skd->starttime[from_m], skd->starttime[to_m], solution->length+i, name);
    if (status) {
...
void ms_log(struct ModSys *ms) {
  ...
  for (int i=0; i<ms->NP; i++) {
    ...
    LOG("[N=%d, b=%d, n=%s, I=%s]\n", ms->cycle[i].N, ms->cycle[i].b, strn.c_str(), strI.c_str());
  }
}
...
int main(int argc, char **argv) {
  ...
  std::string name;
  if (argc != 2) {
    puts("default path is ./input.json");
    name = "input.json";
  } else {
    name = argv[1];
  }
